...
分析结果："""
    ...
    def _extract_section(self, text: str, section_name: str) -> str:
        ...
        # All known section names (Chinese) that act as boundaries
        all_sections = ["研究趋势", "方法论对比", "研究空白", "未来研究方向", "关键发现"]
        other_sections = [s for s in all_sections if s != section_name]
        boundaries = "|".join(other_sections)

        # The heading line may look like:
        #   1. **研究趋势（Research Trends）**：
        #   **研究趋势**：
        #   研究趋势：
        #   ### 研究趋势
        #
        # Match: optional number+dot, optional **, section name, optional (English), optional **, colon
        heading = rf"(?:\d+\.\s*)?\*?\*?{section_name}（[^）]*）\*?\*?\s*[：:]|\*?\*?{section_name}\*?\*?\s*[：:]"

        if other_sections:
            boundary_pat = "|".join(other_sections)
